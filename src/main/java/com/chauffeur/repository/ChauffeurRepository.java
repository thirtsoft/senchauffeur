...
public interface ChauffeurRepository extends JpaRepository<Chauffeur, Long> {
    ...
    // Like  key%  %key  %key%
    Page<Chauffeur> findByDisponibityContaining(String disponibity, Pageable pageable);

    @Query("select count(id) from Chauffeur where addresse.id = ?1")
    long getChauffeurLengthByAddressId(long id);

    @Query("select count(id) from Chauffeur where disponibity LIKE %?1%")
    long getChauffeurSizeByKey(String disponibity);
